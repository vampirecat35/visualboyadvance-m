...
bool SDLAudio::init(long sampleRate) {
    ...
    if (OPTION(kSoundAudioDevice) == _("Default device")) {
        ...
        sound_stream = SDL_OpenAudioDeviceStream(sdl_devices[current_device], &audio, soundCallback, this);
    }
    ...
    samples_buf.reset(static_cast<size_t>(std::ceil(buftime * sampleRate * sizeof(uint16_t))));
...
void SDLAudio::setThrottle(unsigned short throttle_) {
    ...
    coreOptions.throttle = throttle_;
...
void SDLAudio::write(uint16_t* finalWave, int length) {
    ...
    while ((avail = samples_buf.avail() / sizeof(uint16_t)) < samples) {
        samples_buf.write(finalWave, avail * sizeof(uint16_t));
        
        finalWave += avail * sizeof(uint16_t);
        samples -= avail;
    ...
    if (samples > 0) {
        samples_buf.write(finalWave, samples * sizeof(uint16_t));
    }
